# Databricks notebook source exported at Thu, 11 Aug 2016 12:48:12 UTC
# MAGIC %md
...
#I wrote my own udf max rather than build in max which doesn't work here because of naming inconsistancy 
def maxx(l):
  mm = l[0]
  for i in l:
    if(i > mm):
...
df.select(regexp_extract('str', '(\d+)-(\d+)', 2).alias('d')).collect()
...
not200DF = logs_df.filter(logs_df['status'] != lit(200))
not200DF.show(10)
...
# TODO: Replace <FILL IN> with appropriate code
#unique_host_count = logs_df.select(logs_df['host']).distinct().count()#.withColumn('each', lit(1)).agg(col("host"), sum(col("each")))
#it can be solved with dropDuplicates() as well. 
unique_host_count = logs_df.select(logs_df['host']).dropDuplicates().count()
#unique_host_count.show(10, False)
...
#day_to_host_pair_df.show(30, False)
print 'Unique hosts per day:'
daily_hosts_df.show(30, False)
daily_hosts_df.cache()
#daily_hosts_df.take(10)
...
days_with_hosts = daily_hosts_df.select(daily_hosts_df.day).rdd.map(lambda day: day[0]).collect()#<FILL IN>
hosts = daily_hosts_df.select(daily_hosts_df['count']).rdd.map(lambda count: count[0]).collect()#<FILL IN>
...
plt.axis([0, maxx(days_with_hosts), 0, maxx(hosts)+500])
plt.xlabel('Day')
plt.ylabel('Hosts')
...
total_req_per_day_df = logs_df.select(logs_df.host,dayofmonth('time').alias('day')).groupBy('day').count()
total_req_per_day_df.show()
daily_hosts_df2 = daily_hosts_df.select(daily_hosts_df.day, daily_hosts_df['count'].alias('dist_count'))
#daily_hosts_df.show()
#daily_hosts_df2.show()
avg_daily_req_per_host_df = (
  total_req_per_day_df.join(daily_hosts_df2, total_req_per_day_df['day'] ==  daily_hosts_df2['day'], 'inner').drop(daily_hosts_df2['day']).withColumn('avg_reqs_per_host_per_day', total_req_per_day_df['count'] / daily_hosts_df2['dist_count'] ) .drop(daily_hosts_df2['dist_count']).drop(total_req_per_day_df['count'])
)

#print 'Average number of daily requests per Hosts is:\n'
avg_daily_req_per_host_df.show()
avg_daily_req_per_host_df.columns
...
days_with_avg = avg_daily_req_per_host_df.select('day').rdd.map(lambda day: day[0]).collect()
avgs = avg_daily_req_per_host_df.select('avg_reqs_per_host_per_day').rdd.map(lambda day: day[0]).collect()
#for <FILL IN>:
 # <FILL IN>
...
plt.axis([0, maxx(days_with_avg), 0, maxx(avgs)+2])
plt.xlabel('Day')
plt.ylabel('Average')
plt.axhline(linewidth=3, color='#999999')
plt.axvline(linewidth=2, color='#999999')
...
# TODO: Replace <FILL IN> with appropriate code
#display(zip(avgs, days_with_avg))
...
# TODO: Replace <FILL IN> with appropriate code
#not_found_df.show(n=20, truncate=False)
not_found_paths_df = not_found_df.select(not_found_df['path'])
unique_not_found_paths_df = not_found_paths_df.groupBy(not_found_paths_df['path']).count().orderBy(desc("count")).drop('count')
...
# TODO: Replace <FILL IN> with appropriate code
#not_found_paths_df.show(20)
top_20_not_found_df = not_found_paths_df.groupBy(not_found_paths_df['path']).count().orderBy(desc("count"))
...
hosts_404_count_df = not_found_df.select(not_found_df['host']).groupBy(not_found_df['host']).count().orderBy(desc("count"))
...
errors_by_date_sorted_df = not_found_df.select(dayofmonth('time').alias('day')).groupBy('day').count()#.orderBy(desc("count"))
...
days_with_errors_404 = errors_by_date_sorted_df.select('day').rdd.map(lambda day: day[0]).collect()
errors_404_by_day = errors_by_date_sorted_df.select('count').rdd.map(lambda cc: cc[0]).collect()
#for <FILL IN>:
 # <FILL IN>
...
plt.axis([0, maxx(days_with_errors_404), 0, maxx(errors_404_by_day)])
plt.xlabel('Day')
...
hour_records_sorted_df = not_found_df.select(hour(not_found_df['time']).alias('hours')).groupBy('hours').count()#.orderBy('count')
...
hours_with_not_found = hour_records_sorted_df.select(hour_records_sorted_df['hours']).rdd.map(lambda hour: hour[0]).collect()
not_found_counts_per_hour = hour_records_sorted_df.select(hour_records_sorted_df['count']).rdd.map(lambda cc: cc[0]).collect()
...
plt.axis([0, maxx(hours_with_not_found), 0, maxx(not_found_counts_per_hour)])
plt.xlabel('Hour')
plt.ylabel('404 Errors')
plt.axhline(linewidth=3, color='#999999')
plt.axvline(linewidth=2, color='#999999')
